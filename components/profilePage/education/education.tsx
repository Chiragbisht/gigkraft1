...
const Education= () => {
  ...
  const visibleExperiences = showAll ? educationData : educationData.slice(0, 2); // Show only 2 initially
  ...
  return (
    ...
    >
      ...
      <div className="flex px-[25px] flex-row items-center justify-between border-b-[1px] h-[55px] border-solid border-[#D9D9D9] py-3 w-full">
        <h1 className={`${montserrat.className} lg:text-[20px] text-[15px] font-[700]`}>
          Education
        </h1>
        <div className="flex flex-row gap-x-5">
          <Button variant={"ghost"}>
            <Image src={"/images/icons/delete.svg"} alt="delete" width={30} height={30} />
          </Button>
          <Button className="border-primary text-primary rounded-full" variant={"outline"}>
            <span className={`${montserrat.className} text-[12px] font-[700] text-primary`}>
              Edit Education
            </span>
          </Button>
        </div>
      ...
      {educationData.length > 2 && (
        <div className="w-full pb-3 flex items-center justify-center">
          <Button variant="link" className="text-[#4285F4]" onClick={() => setShowAll(!showAll)}>
